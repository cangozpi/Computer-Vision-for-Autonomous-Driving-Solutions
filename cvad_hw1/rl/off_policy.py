...
def collate_fn(data):
    ...
    for i in range(len(data)):
        a.append(data[i][0])
        b.append(data[i][1])
        c.append(data[i][2].detach().cpu().numpy())
        d.append(data[i][3])
        e.append(data[i][4])
        f.append(data[i][5])
        g.append(data[i][6])
...
class BaseOffPolicy(ABC):
    ...
    def test_agent(self, num_trials=5):
        ...
        for _ in range(num_trials):
            ...
            while not is_terminal and ep_step < self.config["episode_max_steps"]:
                ...
                features = [f.detach().cpu().squeeze(0) for f in features] # ADDED BY ME !!! NOT SURE !!!
                action = self.policy(torch.tensor(features).unsqueeze(0), state["command"])
                print("throttle/brake: ", action[:,0].item(), "steer: ", action[:,1].item())
                # Take step
    ...
    def save_checkpoint(self, ckpt_str, *, include_replay_buffer=True):
        ...
        if not os.path.exists("checkpoints/rl"):
